...
@Service
public class PDFCurvaABCService {
	@Autowired
...
public void export(HttpServletResponse response, String nome) throws IOException {
    ...
    */
	 PdfPTable table = new PdfPTable(3);
		...
		hcell = new PdfPCell(new Phrase("Quantidade Vendida", fontTitle));
		hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(hcell);
		
		List<Produto> prod;
		if(nome.isEmpty())	
			prod = produtoRepository.buscarABC();
		else
			prod = produtoRepository.buscarABCLike(nome.toUpperCase());
    ...
    /*	Paragraph paragraph2 = new Paragraph(produto.getCpf() +"                               "+ produto.getCidade()+"                  "+ produto.getNome());
		...
		cell = new PdfPCell(new Phrase(String.valueOf(produto.getQnt_venda())));
		cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
