...
@Service
public class PDFCurvaClienteService {
	@Autowired
...
public void export(HttpServletResponse response, String nome) throws IOException {
    ...
    */
	 PdfPTable table = new PdfPTable(4);
		...
		hcell = new PdfPCell(new Phrase("Total Gasto", fontTitle));
		hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(hcell);
		List<Cliente> prod1;
	...
	if(!nome.isEmpty())
	{
		prod1 = clienteRepository.buscarPorNomenovo(nome.toUpperCase());	
	}
	else
		prod1 = clienteRepository.buscarClientesMais();


   // prod.
    for (Cliente produto : prod1) {  	
    /*	Paragraph paragraph2 = new Paragraph(produto.getCpf() +"                               "+ produto.getCidade()+"                  "+ produto.getNome());
    	...
    	document.add(paragraph2);*/
			...
			cell = new PdfPCell(new Phrase(String.valueOf(produto.getValorcomprado())));
			cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			cell.setHorizontalAlignment(Element.ALIGN_CENTER);
